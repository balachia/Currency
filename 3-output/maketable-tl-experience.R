...
wrap <- function(f,...) {
    f(l=userms[c('basem12a.base','basem12a.l','basem12a.10')],
      ...
      caption='Conditional Logit (Trader/Day): Effects of Leader Experience',
      ...
      custom.model.names=c('(10) Baseline','(11) Log Rank','(12) Rank > 10'),
      reorder.coef=NULL,
      omit.coef='^Leader$',
      custom.coef.names=c('Alter Trade', 'Alter Profit','Leader',
                          'Alter Trade*Leader','Alter Profit*Leader',
                          'Rank','Trade*Rank','Profit*Rank',
                          'Rank*Leader','Trade*Rank*Leader','Profit*Rank*Leader',
                          'Rank', 'Trade*Rank', 'Profit*Rank',
                          'Rank*Leader','Trade*Rank*Leader','Profit*Rank*Leader'),
      ...)
}
...
wrap <- function(f,...) {
    f(l=cpms[c('basem12c.base','basem12c.l','basem12c.10')],
      ...
      digits=3, float.pos='tb',
      caption='Conditional Logit (Currency Pair/Day): Effects of Leader Experience',
      dcolumn=TRUE, booktabs=TRUE, use.packages=FALSE,
      doctype=FALSE, star.symbol='&lowast;',
      include.rsquared=FALSE, include.maxrs=FALSE, include.missings=FALSE,
